import argparse
from combinatorics import solve_combinatorics
from polynomial import solve_polynomial
from calk import solve_calk
...
if __name__ == "__main__":
    ...
    # solve('calk_primefactorsNumber_n=111111')
    # solve('calk_evalWithNumber_quotient=2+0+1+3')
    # solve('polynomial_solveEquation_equation=x+2=6*x-3;variable=x')
    # solve('calk_roundNumber_n=1.123232;comands=2')
    # solve('polynomial_solveTrigonometric_pol=cosh(x)**2+sinh(x)**2')
    # solve('calk_fact_n=33')
    # print('OK')
    # solve('calk_evalWithNumber_quotient=(860-30)/45')
    # solve('calk_findDivisors_n=44')
    # solve('polynomial_solveEquation_equation=b+1=2;variable=b')
    # solve('calk_evalWithNumber_quotient=860-30-18*45')
    solve(input('Введите выражение '))
    print('OK')
